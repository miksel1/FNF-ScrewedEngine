...
/**
 * Just the normal one, but changed it to have more customizable functions
 */
class FlxUINumericStepper extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	public var onPlus:Float -> Void;
	public var onMinus:Float -> Void;
			...
			IsPercent:Bool = false)
	{
		...
		button_plus.onUp.callback = function() {
			if(onPlus != null)
				onPlus(value + stepSize);

			_onPlus();
		};
		button_plus.broadcastToFlxUI = false;

		button_minus.onUp.callback = function() {
			if(onMinus != null)
				onMinus(value - stepSize);
