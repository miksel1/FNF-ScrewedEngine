...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	function getOptions()
	{
		var goption:GameplayOption = new GameplayOption({s: 'Scroll Type', spanish: 'Tipo de Scroll'}, 'scrolltype', 'string', 'multiplicative', ["multiplicative", "constant", "random"]);
		goption.id = 'ST';
		optionsArray.push(goption);

		var option:GameplayOption = new GameplayOption({s: 'Scroll Speed', spanish: 'Velocidad de Scroll'}, 'scrollspeed', 'float', 1);
	...
	public function new()
	{
		...
		{
			var namee = Language.getString(optionsArray[i].name);
			/*if(!namee.endsWith(':') && optionsArray[i].type == 'string') {
				namee = namee + ':';
			}*/
			var optionText:Alphabet = new Alphabet(200, 360, namee, true);
...
class GameplayOption
{
	...
	/**How String/Float/Percent/Int values are shown, %v = Current value, %d = Default value
	*/
