...
class PlayState extends MusicBeatState
{
	...
	public var formatedSong:String = 'test';
	...
	override public function create()
	{
		...
		showCombo = ClientPrefs.data.allowComboSprite;
		showComboNum = ClientPrefs.data.allowNumcomboSprite;
		...
		formatedSong = Paths.formatToSongPath(SONG.song);
		...
		var file:String = Paths.json(formatedSong + '/dialogue'); // Checks for json/Psych Engine dialogue
		if (OpenFlAssets.exists(file))
		{
			dialogueJson = DialogueBoxPsych.parseDialogue(file);
		}

		var file:String = Paths.txt(formatedSong + '/' + formatedSong + 'Dialogue'); // Checks for vanilla/Senpai dialogue
		if (OpenFlAssets.exists(file))
		{
		...
		timeBar.numDivisions = ClientPrefs.data.timeBarDivisions;
		timeBar.alpha = 0;
		...
		generateSong();
		if (!SONG.ghostTappingAllowed && ClientPrefs.data.ghostTapping)
		{
		...
		if (ClientPrefs.data.crazyCounter)
		{ // theoyeah engine?
		...
		if (ClientPrefs.data.showHealth)
		{
			healthTxt = new FlxText(botplayTxt.x, !ClientPrefs.data.downScroll ? botplayTxt.y + 100 : botplayTxt.y - 100, botplayTxt.fieldWidth,
		...
		#if LUA_ALLOWED
		var filesPushed:Array<String> = [];
		var foldersToCheck:Array<String> = [Paths.getPreloadPath('data/' + formatedSong + '/')];

		#if MODS_ALLOWED
		foldersToCheck.insert(0, Paths.mods('data/' + formatedSong + '/'));
		if (Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
			foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/data/' + formatedSong + '/'));

		for (mod in Paths.getGlobalMods())
			foldersToCheck.insert(0,
				Paths.mods(mod + '/data/' + formatedSong + '/')); // using push instead of insert because these should run after everything else
		#end
	...
	{
		return
			(modchartSprites.exists(tag)) ? modchartSprites.get(tag) : (text && modchartTexts.exists(tag)) ? modchartTexts.get(tag) : variables.exists(tag) ? variables.get(tag) : null;
	}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (formatedSong == 'roses' || formatedSong == 'thorns')
		{
			remove(black);

			if (formatedSong == 'thorns')
			{
				add(red);
				camHUD.visible = false;
			}
	...
	function tankIntro()
	{
		...
		tankman.frames = Paths.getSparrowAtlas('cutscenes/' + formatedSong);
	...
	private function generateSong():Void
	{
		...
		var file:String = Paths.json(formatedSong + '/events');
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modsJson(formatedSong + '/events')) || FileSystem.exists(file))
		#else
		if (OpenFlAssets.exists(file))
		#end
		{
			var eventsData:Array<Dynamic> = Song.loadFromJson('events', formatedSong).events;
	...
	override public function update(elapsed:Float)
	{
		...
		for (effect in modchartWiggleEffects)
		...
		else
		{
			if (!paused)
			{
				...
				if (updateTime)
				{
					...
					if (ClientPrefs.data.timeBarType == 'Time Elapsed' || ClientPrefs.data.timeBarType.endsWith(' / Total'))
					{
						songCalc = curTime;
					}
					...
					var finale:String = '';
					if (ClientPrefs.data.timeBarType != 'Song Name') // not lag?
						finale = FlxStringUtil.formatTime(secondsTotal, false);

					if (ClientPrefs.data.timeBarType.endsWith(' / Total'))
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			case 'Dadbattle Spotlight':
				...
				switch (Std.parseInt(value1))
				{
					...
					default:
						...
						dadbattleLight.visible = false;
						defaultCamZoom -= 0.12;
						FlxTween.tween(dadbattleSmokes, {alpha: 0}, 1 / playbackRate, {
							onComplete: function(twn:FlxTween)
							{
								dadbattleSmokes.visible = false;
							}
			...
			case 'Change Scroll Speed':
				if (songSpeedType == "constant" || songSpeedType == "random")
					return;
				var val1:Float = Std.parseFloat(value1);
				var val2:Float = Std.parseFloat(value2);
				if (Math.isNaN(val1))
	...
	public function moveCamera(isDad:Bool)
	{
		...
		else
		{
			...
			if (formatedSong == 'tutorial' && cameraTwn == null && FlxG.camera.zoom != 1)
			{
	...
	function tweenCamIn()
	{
		if (formatedSong == 'tutorial' && cameraTwn == null && FlxG.camera.zoom != 1.3)
		{
		...
		if (ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					var winterHorrorlandNext = (formatedSong == "eggnog");
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (daRating.noteSplash && !note.noteSplashDisabled && ClientPrefs.data.noteSplashes)
		...
		if (cpuControlled || ClientPrefs.data.hideHud)
			return; // fuck it
		...
		rating.visible = showRating; // WHAT THE HECK???
		rating.x += ClientPrefs.data.comboOffset[0];
		rating.y -= ClientPrefs.data.comboOffset[1];

		var comboSpr:Null<FlxSprite> = null;
		if (showCombo)
		{
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			#end if (boyfriend.animation.curAnim != null
				&& boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * boyfriend.singDuration
					&& boyfriend.animation.curAnim.name.startsWith('sing')
					&& !boyfriend.animation.curAnim.name.endsWith('miss'))
				boyfriend.dance();
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (ClientPrefs.data.hitsoundVolume > 0 && !note.hitsoundDisabled && canPlayerLight)
		...
		for (i in 0...achievesToCheck.length)
		{
			...
			if (!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled)
			{
				...
				switch (achievementName)
				{
					...
					case 'debugger':
						if (formatedSong == 'test' && !usedPractice)
						{
							unlock = true;
						}
				}
