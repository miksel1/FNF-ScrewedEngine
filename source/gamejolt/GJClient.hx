...
class GJClient
{
				...
				for (troph in daList)
				{
					if (troph.id == id)
					{
						if (troph.achieved == false)
						{
							...
							printMsg('Trophie "${troph.title}" is already taken by ${getUser()}!');
						break;
					}
				}
			}, function()
				...
				for (troph in daList)
				{
					if (troph.id == id)
					{
						if (troph.achieved != false)
						{
							...
							printMsg('Trophie "${troph.title}" is not taken by ${getUser()} yet!');
						break;
					}
				}
			}, function()
	 ...
	 */
	public static function getScoresList(fromUser:Bool, ?table_id:Int, ?delimiter:Int, ?onSuccess:() -> Void, ?onFail:() -> Void,
			limit:Int = 10):Null<Array<Score>>
	{
	...
	static function urlConstruct(command:String, ?action:String, ?params:Array<Array<String>>, userAllowed:Bool = true, tokenAllowed:Bool = true):Null<Http>
	{
		...
		if (!noDataWarned)
		{
			...
			{
				printMsg('User data was not provided for the command "$curCommand"');
				noDataWarned = true;
			}
		}
