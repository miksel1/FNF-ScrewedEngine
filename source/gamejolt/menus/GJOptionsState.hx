...
class GJOptionsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (!GJClient.hasLoginInfo() && !GJClient.logged)
		{
			...
			logButton = new ActionButton(0, FlxG.height * 0.75, daWidth, daHeight, 'Log In!', function()
			{
		...
		else
		{
			...
			{
				var daButton = new ActionButton(FlxG.width * (((i % 2) + 1) / 3), (FlxG.height * 0.45) + ((daHeight + 25) * Math.floor(i / 2)), daWidth,
					daHeight, optList[i], function()
				{
