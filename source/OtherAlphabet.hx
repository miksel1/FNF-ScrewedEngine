...
class OtherAlphabet extends FlxSpriteGroup
{
	public var text(default, set):String;

	public var bold:Bool = false;
	public var letters:Array<OtherAlphaCharacter> = [];
	...
	public function clearLetters()
	{
		...
		{
			--i;
			var letter:OtherAlphaCharacter = letters[i];
			if(letter != null)
			{
				letter.kill();
		...
		for (character in newText.split(''))
		{
			...
			if(character != '\n')
			{
				...
				if (AlphaCharacter.allLetters.exists(character.toLowerCase()) && (!bold || !spaceChar))
				{
					...
					var letter:OtherAlphaCharacter = new OtherAlphaCharacter(xPos, rows * Y_PER_ROW * scaleY, character, bold, this);
...
class OtherAlphaCharacter extends FlxSprite
{
	...
	public function new(x:Float, y:Float, character:String, bold:Bool, parent:OtherAlphabet)
	{
		super(x, y);
		this.parent = parent;
		image = 'otherAlphabet';
		...
		if(!bold)
		{
			if(AlphaCharacter.isTypeAlphabet(lowercase))
			{
